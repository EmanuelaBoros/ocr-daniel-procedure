...
def get_roc_files(dataset_path, folder_family, lang):
    ...
    list_dir = list(filter(lambda x : os.path.isdir(os.path.join(dataset_path, x)), os.listdir(dataset_path)))
    list_dir.sort()
    
    for dirname in list_dir:
        if folder_family in dirname:
            roc_files.append(dataset_path + "/"  + dirname + "/" + lang + "_roc")
...
def plot(daniel_results):
    for i in range(len(daniel_results)):
        ...
        plt.plot(x_new, y_smooth, label = 'Level: {0}, AUC = {1:0.2f}'.format(i, roc_auc))
